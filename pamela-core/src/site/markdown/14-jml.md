...
<!-- TODO idf -->
1. at method level for preconditions and postconditions.
<!-- TODO idf -->

Following listing shows a basic example of a DbC-based `BankAccount` implementation.
The *Class invariant* are declared using `@Invariant` annotation.
While pre and post condition are respectively declared with `@Requires` and `@Ensures` annotations.
...
public interface BankAccount extends AccessibleProxyObject, SpecifiableProxyObject {
	...
	@Setter(BALANCE)
	@Ensures("balance >= 0") // the field
	@Requires("aBalance >= 0") // the argument
...
A major interest in PAMELA framework use is the dynamic assertion checking at run-time.
When activated, dynamic assertion checking is weaved with the functional behaviour.
<!-- TODO idf -->
If the executed model defines JML contract management clauses (assertions, pre and post conditions) and if the execution configuration forces the assertion checking <!-- TODO idf how --> (API is available through the `SpecifiableProxyObject` interface), following execution are performed:
...
<!-- TODO idf where they are executed -->
