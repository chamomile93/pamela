...
Cloning might be independently performed (use of `cloneObject()` method) or in the context of a subset of an object graph (use of `cloneObject(Object... context}`, where context represents a set of objects which have to be cloned together).
<!-- TODO idf how the cloning happens in case of subset -->
Cloning of an object graph strongly relies on containment closure computation since references to objects should be set according to the cloned object graph.
<!-- TODO confused -->