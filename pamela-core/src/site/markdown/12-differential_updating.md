...
- distance between PAMELA objects is computed by computing the distance between property values for all declared *ModelProperty*, with a ponderation specific to related *ModelProperty* (if type of *ModelProperty* refers to a *ModelEntity*, ponderation equals the number of *ModelProperty* of related type, otherwise ponderation values 1).
<!-- TODO this might need to be rephrased -->