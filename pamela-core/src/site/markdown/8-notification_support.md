...
Default implementation of notification scheme propagates a `PropertyChangeEvent` for each property changing its value, with the former and the new value.
The developper does not have to redefine it for all properties, since all required properties should define a `@PropertyImplementation` annotation.

An interesting point is that this notification scheme may be easily extended by defining a new `PropertyImplementation` (see section on meta-programming).
<!-- TODO add - [Meta-programming support](./pamela-core/3-metaprogramming_support.md) -->
