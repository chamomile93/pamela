# Information Disclosure pattern tutorial

## What is the _Information Disclosure_ pattern?

This security pattern is used to implement a detection of unauthorized disclosure of information mechanism.
<!-- A class diagram of the pattern is given below. -->

![](../../../../../../../../img/authenticator.png)

This pattern verifies that a _Component_ is authorized to exchange messages to another Component using a set of objects known as _Port_ and _Connector_. These objects are used by an _Interaction_ to verify that a component is allowed to receive, send or transfer a _Payload_.

## How to use the pattern?

### Component entity

- Identify the _Component_ class. It is the class whose instance you need to verify.
  - This class must be annotated with `@ModelEntity`.
    - To let the PAMELA Framework know that it is part of your model
  - This class must be annotated with `@Component(patternID = <patternID>)`
    - To declare this model entity as a Component for the Information Disclosure pattern.
    - The `<patternid>` is a String that identify your pattern.
    - You could, for instance, have the following code:

~~~java

@ModelEntity
@Component("InformationDisclosurePattern 1")
...
Once the _Component_, _Port_ and _Connector_ classes are correctly annotated, the PAMELA framework will ensure:

- That every call from Component to Component is authorized per definition.
